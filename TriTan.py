...
class TriTan():
    def __init__(self,
                ...
                svd_atac = None,
                sparse = False,
                n_component= [20,20,50,50]):

        self.clusters = n_clusters
        self.complexity = complexity
        self.precomputed = precomputed
        if svd_rna is not None:
            self.svd_rna= svd_rna
        if svd_atac is not None:
            self.svd_atac= svd_atac
        self.sparse = sparse
    ...
    def fit(self,mdata):
        ...
        iteration=0
        if self.precomputed:
            if np.isnan(self.svd_gene) or np.isnan(self.svd_atac):
                raise ValueError('please input precomputed SVD =[u,s,vt]')
            else:
                u_rna, s_rna, v_rna = self.svd_rna[0],self.svd_rna[1],self.svd_rna[2]
                u_atac, s_atac, v_atac =self.svd_atac[0],self.svd_atac[1],self.svd_atac[2]
                C_rna,D_rna,C_atac,D_atac = self.prepocess_precompute(X_rna,X_atac,u_rna,v_rna,u_atac,v_atac)
                umap_embedding=self.embedding
        else:
            if self.sparse:
        ...
        while True:
            ...
            if iteration >= 20:
                break
        F_new =self.subcluster(F,C_rna,C_atac,umap_embedding)
        while True:
    ...
    def subcluster(self,F,C_rna,C_atac,umap_embedding):
        ...
        for i in range(10):
            ...
            clusterer = hdbscan.HDBSCAN(min_cluster_size=60).fit(embedding)
